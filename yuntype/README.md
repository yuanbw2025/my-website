...
#### 2026-04-20 · 小红书布局插槽系统 + 封面增强 + 配色升级

**核心目标**：让小红书输出从「换个颜色」进化为「换一套结构」——随机按钮现在生成结构完全不同的页面，而不只是换配色。

**新增功能**：

- 🏗️ **块渲染系统（Block Orchestrator）**：提取 4 类独立 variant 渲染器（标题 4 种、段落 3 种、列表 4 种、引用 2 种），每种有各自的视觉语言（卡片/时间轴/气泡/网格/竖条等）。
- 🎲 **骨架 × variant 全量映射**：15 套骨架蓝图（B01–B15）各自对应不同的 block variant 组合，「随机」按钮每次产出真正不同的版式结构，不再只是换色。
- 🖼️ **封面 5 种变体视觉重写**：classic（顶色带+菱形分隔）、bold（左 1/3 主色块+反白标题）、minimal（极细线+大字）、card（悬浮卡片+几何角点）、magazine（杂志版式+首字水印）全部重绘。
- 🎨 **配色快捷入口**：预览工具栏直接暴露「配色」按钮，不再需要在左侧面板里找；弹窗新增 36 色色板（3 行：浅底/鲜明/深沉），非设计背景用户无需手填色值。
- 🇨🇳 **中文编号列表解析**：Markdown 解析器扩展支持 `（1）`、`①`、`一、` 等中文列表标记形式。

---

#### 2026-04-21 · 内容编辑系统上线

**核心目标**：让生成的小红书图片真正可以"改"——不必回去重写 Markdown，在预览界面直接调。

**新增功能**：

- ✏️ **文字内联编辑**（方案 A）：开启「编辑文字」模式后，图上所有文字均可直接点击修改。段落、列表项、引用使用 HTML 模式，保留 `**粗体**`/`*斜体*` 渲染后的 `<strong>`/`<em>` 标签；标题使用纯文本模式，避免 HTML 注入。
- 🧱 **块级编辑面板**（方案 B）：预览右侧出现「块编辑」侧栏，列出当前 content 页的所有内容块（标题/段落/列表/引用），支持 ↑↓ 上下移位、⎘ 复制、× 删除；面板底部提供「+ 标题 / + 段落 / + 列表 / + 引用 / + 分隔」快速追加入口。
- ↶↷ **撤销/重做**：预览工具栏新增撤销/重做按钮，`Ctrl/Cmd+Z` 与 `Ctrl/Cmd+Shift+Z` 快捷键可用，最多保留 50 步操作历史，覆盖文字编辑、块增删重排、模板覆盖、页面拖拽排序四类改动。原始 Markdown 变更后历史自动清空。

**技术改动**：`editable()` 增加 `mode` 参数；orchestrator 为每个元素分配稳定 editId（`p{pageIndex}-e{elIndex}`，列表项追加 `-{itemIndex}`）；Preview 组件引入 callback ref 解决条件渲染下 focusout 监听器不挂载的问题。
...
Phase 9 ✅  → 内容编辑系统：文字内联编辑 + 块增删重排 + 撤销重做
