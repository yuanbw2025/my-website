...
> 状态：ARCHITECTURE BASELINE / 待按阶段施工
> 决策日期：2026-08-05
...
| 世界入口 | 世界引擎页只渲染 `WorldGroupOverview` | 把多世界关系管理误当世界内容 | 建立完整世界工作台；世界组降为“位面与世界结构”模块 |
| 世界内容 | 起源、自然、人文、地图、历史等仍在旧工作区 | 内容存在但产品归属和入口断裂 | 复用既有面板并按世界工作台重新聚合 |
...
| 完整度 | 产品页按正文目标字数估算世界完整度 | 世界与作品语义混淆 | 按模块覆盖、引用完整、验证结果和发布准备度计算 |
| 社区包 | 只分享注册为 `communityShare='world'` 的设定表 | 不包含故事核心、主线、支线、大纲和玩法 | 升级为用户选模块的世界发布清单；正文默认排除 |

代码证据：

- [ProductHubPage.tsx](../src/pages/ProductHubPage.tsx) 的 `projectToWorld()` 把所有项目转换为世界，且完整度来自正文进度。
...
HARNESS-2 不单独等待最后施工。每个 WORLD/PLATFORM 阶段同步补齐：
