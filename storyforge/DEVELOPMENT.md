...
本仓库（`yuanbw2025/storyforge`）是 **开发正本**，所有日常开发、功能迭代、bug 修复都在此仓库进行。
...
- **Vercel**：自动部署，线上地址 `https://yuanbw.vercel.app/storyforge/`
...
```bash
cd ~/Desktop/projects/storyforge
...
或使用主库的 `sync.sh` 脚本一键同步：

```bash
cd ~/Desktop/projects/my-website
bash sync.sh storyforge
...
> **架构变更记录**：2026-05-26 之前采用"主库开发 → 镜像推送"模式（即在 my-website 中开发，通过 subtree push 推送到本仓库）。现已改为"独立仓库开发 → 主库集成"模式。
