...
describe('WORLD-1 · v43 历史主入口归并', () => {
  ...
  it('只桥接同世界的空目标，已有正式历史不覆盖且旧字段原样保留', async () => {
    db.close()
    await Dexie.delete('storyforge')
    ...
    expect(worldviews.find(row => row.worldGroupId === 11)?.historyLine).toBe('甲世界旧史')
    expect(worldviews.find(row => row.worldGroupId === 22)?.worldEvents).toBe('乙世界旧事')
  })
})
