...
**定位**：「已写正文 → AI 提取 → 结构化产物」的下游提取。主角随剧情成长，AI 自动检测并记录主角当前到达的**力量体系阶段**，展示当前境界 + 一路晋升历程。

**现状缺口**（已确认）：现有状态表的状态提取是自由 key-value，**可能**顺手记一条"境界:金丹"，但：① 不专门、不保证；② 完全不读世界观「力量体系」定义的阶梯，无法判断"当前在第几阶/距下一阶差什么"；③ 无结构化的当前阶段 + 晋升历程展示。

**设计要点**：
- **挂钩力量体系定义**：提取时把世界观「力量体系」（`powerHierarchy`）的等级阶梯作为参照喂给 AI，才能判断当前阶段在阶梯中的位置
- **主角为主**：默认追踪主角，可扩展到其他重要角色
- **进度 + 历程**：当前阶段 + 每次突破的章节/触发条件（时间线式，像"修为进度条"）
- **多世界适配**：诸天流每个世界力量体系不同，按当前世界的体系判断；与世界组「能力限制」字段呼应（主角跨世界被压制）
- **可反哺写作**：主角当前境界可作为上下文注入后续章节，避免 AI 写出境界倒退硬伤
- **复用**：与即将重构的「自然环境·灵兽异兽词条」共用同一套力量体系阶梯参照（见 Phase 35 异兽词条的「力量体系等级」字段）
...
> 来源：用户构想（2026-06-03） | 状态：**需求已确认，完整设计文档待写/待开工确认** | 规模：大重构（动 DB + 核心世界观面板 + 数据迁移）
...
- ⚔️ 势力：类型(门派/朝廷/商会/部落) / 势力范围 / 领导者 / 核心成员 / 实力等级 / 宗旨目标 / 敌友关系 / 标志旗帜
...
**待拍板的取舍点**：
1. **势力词条 vs 现有阵营(Faction)**：建议合并为一套，避免双轨
2. **世界历史线 vs 历史年表**：是否把历史线并进已词条化的历史年表
3. **已填自由文本的迁移**：保留旧文本作"概述" + 新建词条，或 AI 辅助把旧文本拆成词条
4. 与 Phase 34（力量阶段）共用「力量体系阶梯」参照

**风险**：动 DB（新增词条表 + 类型 schema）+ 核心世界观面板重写 + 现有 ItemSystem/NaturalResources 数据迁移。需先写完整设计文档，再分类逐步迁移，每步可验证。
