...
>
> **最后更新**: 2026-06-18（6 月 17 日 bug 批次 26 项已全部完成；D-1/R1 已实现，待审查部署）
...
> **目标**：让小说写到几十万、甚至百万字后，剧情/人设/设定依然自洽不漂移。这是工具能否成为"长篇工具"的命门。
> **现状一句话**：当前是优秀的"设定管理器"，但"剧情连续性"这条链基本是断的（前情不前喂、正文↔章纲解耦、无滚动前情、状态库手动）。
> **方向（判断已定）**：以"**显式状态库（带章号时序）+ 每次注入 + 生成后校验**"为确定性主干（更保险、复用现有状态卡/伏笔/年表）；RAG/embedding 作为细节召回副手；超长上下文 + 上下文缓存作为降本效率层。
> **完整方案与技术优劣判断**：见独立文档（作者桌面）《**StoryForge_长期一致性目标实现方案.md**》——含现状评估、技术选型对比、分阶段落地、浏览器/BYOK 工程容灾、风险清单。**待外部审核后再决定是否立项排期**，本 ROADMAP 暂只挂目标。
